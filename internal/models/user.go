...
func (m *UserModel) CreateUser(name string, email string, role string) error {
	users, err := m.SearchUser(email, "email")
	if err != nil {
		return err
	}
	if len(users) > 0 {
...
func (m *UserModel) GetUserById(id int) (*User, error) {
	user := &User{}
	err := m.DB.QueryRow("SELECT * FROM users WHERE id = $1", id).Scan(&user)
	if err != nil {
		return nil, err
	}
	return user, nil
...
func (m *UserModel) SearchUser(fieldValue string, field string) ([]*User, error) {
	rows, err := m.DB.Query("SELECT * FROM users WHERE $1 = $2", field, fieldValue)
	if err != nil {
		return nil, err
	}
	defer func(rows *sql.Rows) {
